...
public class RelicPlayerController : MonoBehaviour {
...
//            };
    ...
    //VERSION WITH THE SPINE ANIMATION
    //public void MovePlayer( Vector3 worldCoordinates, CharacterAnimations.AnimationList animation ){
    ...
    public void MovePlayer( Vector3 worldCoordinates ) {
        if ( this.ShouldChangeDirectionFacing( worldCoordinates.x ) ) {
            ChangeDirectionFacing( );
        }
        monsterPosition = NewMonsterPosition( worldCoordinates );
        travelDistance = FindDistanceToTravel( );
        //StartMovementAnimation( animation );
        StartCoroutine( TweenCharacterMovement() );
    }

	Vector3 NewMonsterPosition( Vector3 newDestination ) {
		return new Vector3( newDestination.x, newDestination.y, transform.position.z );
	}

	float FindDistanceToTravel( ) {
		return Vector3.Distance( transform.position, monsterPosition );
	}

	bool ShouldChangeDirectionFacing( float positionMovingTowards ) {
		return ( transform.localScale.x > 0.0f &&
		    transform.position.x < positionMovingTowards ) ||
	...
	void ChangeDirectionFacing( ) {
		Vector3 changedDirection = transform.localScale;
		changedDirection.x = -changedDirection.x;
		transform.localScale = changedDirection;
	}
    ...
    IEnumerator TweenCharacterMovement( ){
		for( float i = travelDistance; i > STOP_WITHIN_RANGE; ) {
			float speed = ( ( 1.5f * Time.deltaTime ) / travelDistance );
			travelDistance = Vector3.Distance( transform.position, monsterPosition );
			if( travelDistance <= STOP_WITHIN_RANGE ) {
			    StopMovement( );
			}
			i = travelDistance;
			transform.position = Vector3.Lerp( transform.position, monsterPosition, speed );
