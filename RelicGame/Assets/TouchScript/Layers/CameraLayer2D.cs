...
namespace Assets.TouchScript.Layers {
    ...
    public class CameraLayer2D : CameraLayerBase {
        ...
        protected override LayerHitResult castRay( Ray ray, out TouchHit hit ) {
            hit = new TouchHit( );
            var hits = Physics2D.GetRayIntersectionAll( ray, float.PositiveInfinity, this.LayerMask );

            if( hits.Length == 0 ) return LayerHitResult.Miss;
            if( hits.Length > 1 ) hits = this.SortHits( hits );
        ...
        private void OnEnable( ) {
            this.sortedHits = new List<RaycastHit2D>( );
        }

        private RaycastHit2D[ ] SortHits( RaycastHit2D[ ] hits ) {
            this.sortedHits.Clear( );
            this.sortedHits.AddRange( hits );
            this.sortedHits.Sort( ( a, b ) => {
            ...
            return this.sortedHits.ToArray( );
        }
